...
- **배포 파일**: [`policies/jump.onnx`](../policies/jump.onnx) (체크포인트: Run 41 `model_1550.pt`)
- **주요 성능 지표** (BAM m6 50 Hz CPU 실측):
  - **최고 도약 고도**: 몸통 정점 $Z = 147.1\text{ mm}$
  - **발 체공 클리어런스**: **왼발 $67.3\text{ mm}$ (6.7 cm)**, **오른발 $54.1\text{ mm}$ (5.4 cm)**
  - **착지 후 기립 안정성**: **Pitch $+0.1^\circ$, Roll $+0.2^\circ$, $V_z = 0.001\text{ m/s}$** (전복률 0%)
  - **카운터무브먼트 시퀀스**: 깊은 웅크림($Z=81\text{ mm}$) → 폭발 도약($+0.59\text{ m/s}$) → 6.7cm 체공/턱 → 무반동 댐핑 착지
