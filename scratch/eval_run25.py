...
def evaluate_run25(onnx_path, name="Run 25"):
    print(f"\n{'='*75}")
    ...
    for step in range(50): # 1.0s @ 50 Hz
        ...
        # left knee: qpos[7+3], right knee: qpos[7+12]
        # Microduck convention: forward flexion = positive for left, negative for right
    ...
    # Summary
    crouch_min_z = min(r['trunk_z'] for r in records[:10])
    max_vz = max(r['vz'] for r in records[:15])
    ...
    print("\n--- KPI SUMMARY ---")
    print(f"Crouch Dip:          {z_stand*1000.0 - crouch_min_z:.1f} mm")
    ...
    max_rk_hyp = max(r['rk_deg'] for r in records[8:25])
