...
def evaluate_policy(name, onnx_path, jump_dur=0.55):
    ...
    for step in range(60): # 1.2s
        ...
        if step >= 6: # after crouch dip
            if is_air:
                ...
                # Left knee: forward bend is > 0
                lk_fwd = max(0.0, data.qpos[7+3])
                # Right knee: forward bend is < 0 (so -qpos is positive)
    ...
    print(f"Aerial Forward Knee Tuck: L={max_lknee_fwd:.2f} rad, R={max_rknee_fwd:.2f} rad")
...
if __name__ == '__main__':
    ...
    if os.path.exists('scratch/jump_run18.onnx'):
        evaluate_policy('Run 18 (Symmetric Attempt)', 'scratch/jump_run18.onnx', jump_dur=0.55)
    if os.path.exists('scratch/jump_run19.onnx'):
        evaluate_policy('Run 19 (Directional Tuck & Anti-Hyperextension)', 'scratch/jump_run19.onnx', jump_dur=0.55)
