...
def record_jump(onnx_path, out_gif="scratch/jump_rollout.gif", total_steps=75): # 1.5s total (50 steps jump + 25 steps standing recovery)
    ...
    print(f"\nRecording {total_steps} steps ({(total_steps*0.02):.2f}s) of jump rollout...")
    print(f"{'Step':<5} | {'t(s)':<5} | {'TrunkZ(mm)':<10} | {'Vz(m/s)':<8} | {'Pitch(°)':<9} | {'Roll(°)':<8} | {'MinClr(mm)':<10} | {'Status'}")
    print("-" * 80)
    ...
    for step in range(total_steps):
        ...
        status = "JUMPING" if step < 50 else "STANDING"
        if abs(pitch_deg) > 40.0 or abs(roll_deg) > 40.0 or trunk_z < 60.0:
            status = "FALLEN!"

        if step % 5 == 0 or status == "FALLEN!":
            print(f"{step:<5} | {t:<5.2f} | {trunk_z:<10.1f} | {vz:<+8.3f} | {pitch_deg:<+9.1f} | {roll_deg:<+8.1f} | {min_clr:<10.1f} | {status}")

    # Save animated GIF (50 Hz = 20ms per frame)
    if frames:
        frames[0].save(out_gif, save_all=True, append_images=frames[1:], duration=40, loop=0) # 25 fps display for smoothness
        print(f"\nSaved rollout recording to: {out_gif} ({len(frames)} frames)")

if __name__ == '__main__':
    onnx = sys.argv[1] if len(sys.argv) > 1 else 'policies/jump.onnx'
    gif = sys.argv[2] if len(sys.argv) > 2 else 'scratch/jump_rollout.gif'
    record_jump(onnx, gif)
