...
def test_ankle_knee_jump():
    ...
    # Full extension thrust:
    # Knee fully straight (0.0), and ankle plantarflexion (pushes toes down, ankle angle = 0.0 or slightly negative)
    push_qpos = default_qpos.copy()
    push_qpos[2] = -0.30        # hip extends forward
    push_qpos[3] = 0.00         # knee straight
    push_qpos[4] = 0.10         # ankle pushes off toes!
    push_qpos[11] = +0.30
    push_qpos[12] = 0.00
    push_qpos[13] = -0.10

    # Aerial tuck: knees pulled up towards chest, ankles tucked
    ...
    max_vz = max(r['vz'] for r in records[:22])
