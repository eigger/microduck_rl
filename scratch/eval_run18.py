...
def evaluate_policy(name, onnx_path, jump_dur=0.55):
    ...
    for step in range(60): # 1.2s
        ...
        if step >= 6: # after crouch dip
            if is_air:
                ...
                lk = abs(data.qpos[7+3])
                rk = abs(data.qpos[7+12])
                if lk > max_lknee: max_lknee = lk
                if rk > max_rknee: max_rknee = rk
            else:
    ...
    print(f"Aerial Knee Tuck: L={max_lknee:.2f} rad, R={max_rknee:.2f} rad (Symmetry: {abs(max_lknee-max_rknee):.2f})")
...
if __name__ == '__main__':
    ...
    if os.path.exists('scratch/jump_run17.onnx'):
        evaluate_policy('Run 17 (Apex Rise)', 'scratch/jump_run17.onnx', jump_dur=0.55)
    if os.path.exists('scratch/jump_run18.onnx'):
        evaluate_policy('Run 18 (Bilateral Tuck)', 'scratch/jump_run18.onnx', jump_dur=0.55)
